...
- [x] Replace hardcoded CSS/JS paths with `$this->directoryName()` for reliable installation across all Bludit configurations
...
- [ ] Strengthens the protection of runtime parameter synchronization
- [ ] Complete admin-side i18n by replacing remaining hardcoded French strings in JavaScript (confirmation prompt and pagination labels) with translation keys.
...
- [x] Altcha algorithm selector in the settings page (SHA-256, SHA-384, SHA-512)
...
- [ ] Altcha algorithm selector in the settings page (Argon2ID, Scrypt) : need upgrade Altcha to version 3
